...
| `flights_to_listing` | integer | Number of flights of stairs from street-level entry to reach the listing. `0` = entry level, no stairs. Counts flights to the listing specifically, not flights in the building overall. Convention-agnostic — consistent across UK, US, and all other regional floor-numbering systems. |
...
**6.14 Stairs and lift:** Where `accessibility.flights_to_listing` is greater than `0` and `accessibility.lift_available` is `false` or absent, agents must surface this as an accessibility note when presenting the listing to users who have indicated mobility requirements.
...
| 0.1.5-draft | March 2026 | `capacity.guests_max` renamed to `capacity.guests` (breaking field rename — aligns RFC with live index validator and Walled Garden Hut reference implementation); `accessibility.flights_to_listing` added as optional integer (number of stair flights from street-level entry to listing, `0` = entry level, convention-agnostic across UK, US, and other regional floor-numbering systems); `accessibility.floors_in_building` added as optional integer (total levels from street-level entry); agent behaviour guideline 6.14 added covering stairs and lift accessibility surfacing |
